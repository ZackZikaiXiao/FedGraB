...
def hook_func_tensor(grad):
    # for classifier_id in range(len(grad)):  # 对于每个分类器的梯度
    #     if classifier_id == self.gt_classes[classifier_id]: # 正样本
    #         grad[classifier_id] *= self.pos_w               # 加权
    #     else:
    #         grad[classifier_id] *= self.neg_w               # 加权
    print(grad)
...
if __name__ == '__main__':
    ...
    epoch_loss = []
    balance_softmax = BalancedSoftmax([5000, 3584, 2569, 1842, 1320, 946, 678, 486, 348, 250])
