...
from util.update_baseline import LocalUpdate
...
writer = SummaryWriter('runs_tensorboard/pid_activ')
...
#         # head就不改了，不然性能血崩
#         self.weight[torch.arange(self.n_i), :10] = 1
...
#         return self.loss_weight * cls_loss
    ...
    #     # print(glo_itr)
    #     for i in range(10):
    #         writer.add_scalar("diff " + str(i), self.pn_diff[i], global_step=glo_itr)
...
if __name__ == '__main__':
    ...
    localUpdate_obj = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[0])   # 就训练第一个client吧
    ...
    epoch_loss = []
    pid_loss = PIDLOSS(device=args.device, pidmask=["head"], class_activation=False)

    client_distribution_0 = datasetObj.training_set_distribution[0]   # 第0个client的分布
    three_shot_dict, _ = shot_split(client_distribution_0, threshold_3shot=[75, 95])
    pid_loss.get_3shotclass(head_class=three_shot_dict["head"], middle_class=three_shot_dict["middle"], tail_class=three_shot_dict["tail"])
    for epoch in tqdm(range(epochs)):
        # train
        batch_loss = []
        # distri_class_real = [0 for i in range(10)]
        # use/load data from split training set "ldr_train"
        for images, labels in localUpdate_obj.ldr_train:
            ...
            # print(glo_itr)
            for i in range(10):
                writer.add_scalar("diff " + str(i), pid_loss.pn_diff[i], global_step=glo_itr)

        epoch_loss.append(sum(batch_loss)/len(batch_loss))

        # test
        acc_s2, global_3shot_acc = globaltest(copy.deepcopy(net).to(args.device), dataset_test, args, dataset_class=datasetObj)
        global_3shot_acc_mean = (global_3shot_acc["head"]+global_3shot_acc["middle"]+global_3shot_acc["tail"]) / 3
        ...
        # acc_local, f1_macro, f1_weighted, acc_3shot_local = localtest(net, dataset_test, dataset_class = datasetObj, idxs=dict_localtest[0], user_id = 0)
        # print('epoch %d, global test acc  %.3f \n' % (epoch, acc_local))
        # print('epoch %d, global 3shot acc: [head: %.3f, middle: %.3f, tail: %.3f]' % (epoch, acc_3shot_local["head"][0], acc_3shot_local["middle"][0], acc_3shot_local["tail"][0]))
