...
if __name__ == '__main__':
    ...
    netglob = torch.load("/home/zikaixiao/zikai/aaFL/fl_gba_cifar100/output/if100/netglob_150.pth", map_location='cpu')
    ...
    # netglob.load_state_dict(torch.load("/home/zikaixiao/zikai/aaFL/FedIR/save/model_499.pkl", map_location='cpu'))
    ...
    for rnd in tqdm(range(args.rounds)):
        ...
        for idx in range(args.num_users):  # training over the subset, in fedper, all clients train
            ...
            labels = set(dataset_train.targets)
                    # datasets['train'].targets 公共数据集
            p = torch.tensor([(torch.tensor(dataset_train.targets) == label).sum() for label in labels]) / len(dataset_train.targets)
            # p = torch.sigmoid(p)
            # p = torch.tensor([(torch.tensor(datasets['train'].targets) == label).sum() for label in labels]) / len(datasets['train'].targets)
            numerator = torch.tensor([(torch.tensor(dataset_train.targets)[list(dict_users[idx])] == label).sum() for label in labels])
            # numerator = numerator.sort()
            min_temp = 1e10
            for i in range(numerator.shape[0]):
                if numerator[i] != 0 and numerator[i] <= min_temp:
                    min_temp = numerator[i]

            for i in range(numerator.shape[0]):
                if numerator[i] == 0:
                    numerator[i] =  min_temp
            del min_temp

            q = numerator / len(torch.tensor(dataset_train.targets)[list(dict_users[idx])])
            # q = torch.sigmoid(q)
            ...
            criterion_fedir = CrossEntropyLoss(weight=weight).cuda(args.gpu)


            w_local, loss_local = local.update_weights_fedir(net=copy.deepcopy(netglob).to(args.device), seed=args.seed, net_glob=netglob.to(args.device), epoch=args.local_ep, criterion = criterion_fedir)
