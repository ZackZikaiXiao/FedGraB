...
if __name__ == '__main__':
    ...
    # pid init
    pid_losses = [PIDLOSS(device=args.device, pidmask=["head", "middle"], class_activation = True) for i in range(args.num_users)] # 每一个client都有一个pidloss
    # 算出head, middle和tail
    ...
    for idx in range(args.num_users):
        pid_losses[idx].get_3shotclass(head_class=[0, 1], middle_class=[2, 3, 4, 5], tail_class=[6, 7, 8, 9])
    ...
    for rnd in tqdm(range(args.rounds)):
        ...
        for idx in range(args.num_users):  # training over the subset, in fedper, all clients train
            ...
            w_local, loss_local = local.update_weights_pid(net=copy.deepcopy(netglob).to(args.device), seed=args.seed, net_glob=netglob.to(args.device), epoch=args.local_ep, pidloss = pid_losses[idx])
