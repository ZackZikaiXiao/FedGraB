...
class PIDLOSS(nn.Module):
    ...
    def hook_func_tensor(self, grad):
        ...
        for c_id in range(classes_num):
            if c_id in self.tail_class:     # 按照id
                if random_unit(prob_dist[tail_id]) == True:
                    # 忽略
                    self.weight[torch.arange(batchsize), c_id] = 1  # 默认了lt顺序
                    tail_id += 1
        ...
        for c_id in range(classes_num):
            if c_id in select_record:
                grad_temp[torch.arange(batchsize), c_id] = 0  # 默认了lt顺序
    ...
    def get_weight(self, target):
        ...
        for i in range(self.num_classes):       # 对于十个类别中的第i个
            pid_out = self.controllers[i].PID_calc(self.pn_diff[i], 0) ################## 修改pid的跟踪值
